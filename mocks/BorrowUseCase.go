...
import (
	...
	response "git.garena.com/sea-labs-id/bootcamp/batch-02/shared-projects/library-api/entity/payloads/response"
)
...
func (_m *BorrowUseCase) CreateBorrowRecord(ctx context.Context, borrow *models.Borrow) (*response.BorrowResponse, error) {
	ret := _m.Called(ctx, borrow)

	var r0 *response.BorrowResponse
	if rf, ok := ret.Get(0).(func(context.Context, *models.Borrow) *response.BorrowResponse); ok {
		r0 = rf(ctx, borrow)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*response.BorrowResponse)
		}
	}
